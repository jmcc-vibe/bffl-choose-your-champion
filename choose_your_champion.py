...
DB_PATH = os.environ.get("CYC_DB_PATH", "choose_your_champion.db")
    ...
    st.markdown(
        ...
        .stApp::before {
          ...
          background:
            ...
            radial-gradient(900px 200px at 50% -10%, rgba(91,140,255,0.10), transparent 60%);
          mix-blend-mode: overlay;
        }
        /* BFFL wordmark styles */
        .bffl-title {
          margin: 0; font-size: 1.9rem; font-weight: 800; letter-spacing: 0.3px;
          background: linear-gradient(90deg, var(--accent), #9AE6B4 70%);
          -webkit-background-clip: text; background-clip: text; color: transparent;
          text-shadow: 0 2px 18px rgba(91,140,255,0.35);
        }
        .bffl-sub { margin: 0.25rem 0 0; color: #a0a8b8; font-weight: 600; }
        /* Sticky tabs for better mobile nav */
        div[role='tablist'] {
        ...
        @media (max-width: 640px) {
          ...
          .bffl-title { font-size: 1.4rem; }
          .bffl-sub { font-size: 0.95rem; }
...
def ui_weekly_results():
    ...
    week = st.number_input("Week", min_value=1, max_value=30, value=1, step=1, key="results_week")
    ...
    if not cons_wk.empty:
        ...
        # Who picked whom (named) for a selected matchup
        matchups = cons_wk[['matchup_id','game_slug','team_a','team_b']].drop_duplicates().sort_values('matchup_id')
...
def ui_analytics():
    ...
    if um['graded_consensus'] == 0:
        ...
        c = alt.Chart(um['weekly_rates']).mark_line(point=True).encode(
            ...
            tooltip=['week','upset_rate']
        ).properties(height=280)
        st.altair_chart(c, use_container_width=True)
        st.markdown("**Top Upset Hunters**")
        st.dataframe(um['manager_upset_wins'])

    st.divider()
    st.markdown("**League Team Pick Tendencies**")
    tpt = team_pick_tendencies()
    if tpt.empty:
        st.info("Shows up after some picks.")
    else:
        st.dataframe(tpt[["team","times_picked","times_graded","correct_picks","pick_acc"]])
        sc = alt.Chart(tpt).mark_circle().encode(
            x=alt.X('times_picked:Q', title='Times picked'),
            y=alt.Y('pick_acc:Q', title='Accuracy when picked', axis=alt.Axis(format='%')),
            size='times_picked:Q',
            tooltip=['team','times_picked','pick_acc']
        ).properties(height=300)
        st.altair_chart(sc, use_container_width=True)

    st.divider()
    st.markdown("**Manager Team Bias**")
    conn = get_conn()
    managers_df = pd.read_sql_query("SELECT name FROM managers ORDER BY name", conn)
    mgrs = managers_df['name'].tolist() if not managers_df.empty else []
    chosen_manager = st.selectbox("Select manager", options=mgrs)
    if chosen_manager:
        mb = manager_team_bias(chosen_manager)
        if mb.empty:
            st.info("No picks yet for this manager.")
        else:
            st.dataframe(mb[["team","times_picked","times_graded","correct_picks","pick_acc"]])

    st.divider()
    st.markdown("**Cumulative Win% by Week (select managers)**")
    cum = weekly_manager_cumulative()
    if not isinstance(cum, pd.DataFrame) or cum.empty:
        st.info("Appears after first graded week.")
    else:
        latest = cum.sort_values('week').groupby('manager').tail(1).sort_values('cum_correct', ascending=False)
        default_mgrs = latest['manager'].head(5).tolist()
        sel = st.multiselect("Managers to chart", options=latest['manager'].tolist(), default=default_mgrs)
        if sel:
            plotdf = cum[cum['manager'].isin(sel)]
            lc = alt.Chart(plotdf).mark_line(point=True).encode(
                x=alt.X('week:O', title='Week'),
...
def main():
    ...
    ui_header()
    ui_style_panel()
    inject_css(st.session_state.get('theme','Dark'), st.session_state.get('accent', PRIMARY_DEFAULT))
    inject_extra_css()
    ui_login()

    user = st.session_state.get("user")

    tabs = ["Picks", "Weekly Results", "Leaderboard", "Analytics", "Admin"] if user and user.get("is_admin") else ["Picks", "Weekly Results", "Leaderboard", "Analytics"]
    t = st.tabs(tabs)

    with t[0]:
        if user:
            st.markdown("<div class='cyc-card'>", unsafe_allow_html=True)
            ui_make_picks(user)
            st.markdown("</div>", unsafe_allow_html=True)
        else:
            st.info("Sign in to make picks.")
    with t[1]:
        ui_weekly_results()
    with t[2]:
        ui_leaderboard()
